...
// create thumbnails
    ...
    product.variants.forEach((variant, index) => {
      ...
      if (!variant.in_stock || product.stock_ml < variant.ml) {
    ...
    purchaseBox.innerHTML = `
      <div class="purchase-box">
        ...
        <div style="display:flex; gap:10px; justify-content:center; flex-wrap:wrap;">
          ...
          <button class="primary-btn" onclick="addToCart('${product.id}', '${product.name}', '${selectedVariant.size}', ${selectedVariant.price}, '${product.image}', ${selectedVariant.ml})">
            Add to Cart
          </button>
...
async function sendOrderWhatsApp(subtotal, shipping, total) {
  closeCart();
  ...
  async function updateStockAfterOrder() {
    try {
      for (const item of cart) {
        ...
        // Calculate how much ml to subtract
        const usedML = item.ml * (item.qty || 1);

        // Get current product from your loaded list
        const product = allProducts.find(p => p.id === item.id);

        if (!product) continue;

        const newStock = Math.max(0, (product.stock_ml || 0) - usedML);

        await updateDoc(productRef, {
          stock_ml: newStock
        });

        console.log(`Updated ${product.name}: ${newStock}ml left`);
      }
...
function addToCart(id, name, size, price, image, ml) {
